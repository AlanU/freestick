...
const FSBaseDevice* FSBaseManager::getDevice(const elementID deviceID)
{
...
void  FSBaseManager::ListenForAllJoysticksForEventTypes(u32 eventFlags,IFSJoystickListener & listener)
{
    if(eventFlags & FS_JOYSTICK_CONNECTED_EVENT)
    {
        if(!deviceMap.empty())
        {
            std::unordered_map<u32, FSBaseDevice * >::iterator itr;
...
void  FSBaseManager::UnListenForAllJoysticksForEventTypes(u32 eventFlags,IFSJoystickListener & listener)
{
    if(eventFlags & FS_JOYSTICK_CONNECTED_EVENT)
    {
...
elementID FSBaseManager::getNextID() const
{
    static u32 ID = 1;//0 is reserved for errors
